...
- [ ] 캐시된 위치정보가 없을때만 강남역 잡히게 처리해야함
- [x] 위치정보를 30일동안 캐싱 하게 해야함
- [ ] 캐시가 있다면 먼저 캐시된 위치를 보여주고, 팝업을 물어보지 않는 경우에 한해서 자동으로 현 위치를 갱신할수 있게 구현 시도해볼것
- [ ] 현 위치 잡힐때 지도 이동 시켜야함
- [ ] 모든 지도 포커스 이동에 소수점 개수 방식 구현해야함 ( 소수점 개수가 많을수록 확대가 되게 (레벨이 낮게) 구현하고, 좌표 검색 시키고 바운드 되도록 구현해야함 )
- [ ] 현 위치가 있을때 좌표 검색 구현해야함
- [ ] 현 위치가 있고 진입시 list 검색인 경우에 대해 소수점 개수 방식으로 포커스 이동시켜야함
- [ ] 현 위치가 있고 id 검색인 경우에 대해서는 현 위치 관련 처리는 지역명만 해줘야함
